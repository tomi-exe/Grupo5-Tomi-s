  ...
  static async createCoupon(data: CreateCouponData): Promise<ICoupon> {
    ...
    try {
      ...
      const result = await session.withTransaction(async () => {
        ...
        const event = await (Event as mongoose.Model<any>)
        ...
        console.log(
          `✅ Cupón creado: ${coupon.code} para evento "${event.eventName}"`
        );
    ...
    try {
      ...
      if (!canUse) {
        return {
          isValid: false,
          reason:
            "No puedes usar este cupón. Verifica que tengas un ticket para este evento y no lo hayas usado antes.",
        };
      }
  ...
  static async applyCoupon(data: ApplyCouponData): Promise<ApplyCouponResult> {
    ...
    try {
      ...
      const result = await session.withTransaction(async () => {
        ...
        console.log(
          `💰 Cupón aplicado: ${coupon.code} - Descuento: $${discountAmount} - Usuario: ${data.userId}`
        );
  ...
  static async getUserEventCoupons(userId: string, eventId: string) {
    try {
      ...
      const userTicket = await Ticket.findOne({
        $or: [{ userId: userId }, { currentOwnerId: userId }],
        eventId: eventId,
        status: "active",
      });
      ...
      const usedCouponIds = (await CouponUsage.find({ userId }).distinct(
        "couponId"
