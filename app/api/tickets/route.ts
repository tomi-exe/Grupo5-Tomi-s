...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ message: "Compra registrada con éxito", ticket }, { status: 201 });
  } catch (error) {
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (!ticketId || !newUserId) {
      return NextResponse.json({ message: "Faltan datos para transferir" }, { status: 400 });
    }
    ...
    return NextResponse.json({ message: "Transferencia realizada", ticket }, { status: 200 });
  } catch (error) {
